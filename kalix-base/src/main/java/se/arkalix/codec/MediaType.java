 ...
 * <p>
 * Media type specifiers differ from {@link CodecType} instances in two major
 * ways. Firstly, each media type always specifies a major type, which
 * classifies it as a member of an abstract type category. Secondly, they can
 ...
 */
public final class MediaType implements ToCodecType {
    private static final ConcurrentHashMap<CodecType, MediaType> codecTypeToMediaType = new ConcurrentHashMap<>();
    ...
    /**
     * Gets cached media type matching given codec, or creates a new one
     * using a simple heuristic.
     *
     * @param codecType Codec associated with desired media type.
     * @return Cached or new codec.
     */
    @ThreadSafe
    public static MediaType getOrCreate(final CodecType codecType) {
        Objects.requireNonNull(codecType, "codec");
        final var mediaType = codecTypeToMediaType.get(codecType);
        if (mediaType != null) {
            return mediaType;
        }
        ...
        {
            if (!codecType.isGeneral() && codecType.isTextual()) {
                type = "text";
                if (codecType.isCharset()) {
                    subtype = "plain";
                    final var charsetName = codecType.name().toLowerCase();
            ...
            subtype = codecType.name().toLowerCase();
            original = type + "/" + subtype;
            parameters = Map.of();
        }

        final var newMediaType = new MediaType(original, type, subtype, null, parameters, codecType);
        final var oldMediaType = codecTypeToMediaType.putIfAbsent(codecType, newMediaType);

        return oldMediaType == null ? newMediaType : oldMediaType;
    }

    /**
     * Registers given media-type as the preferred match for the codec it
     * contains.
     *
    ...
    public static MediaType register(final MediaType mediaType) {
        ...
        Objects.requireNonNull(mediaType.codecType, "mediaType.codec()");

        codecTypeToMediaType.put(mediaType.codecType, mediaType);
     ...
     * <p>
     * The suffix, if present, is intended to identity the structural codec
        ...
        error:
        {
            ...
            if (suffix == null) {
                if (subtype.regionMatches(true, subtype.length() - 4, "-exi", 0, 4)) {
                    codecType = CodecType.EXI;
                    break codec;
                }
                final var codec0 = CodecType.getOrCreate(subtype);
                if (!codec0.isRegistered() && type.equalsIgnoreCase("text") && subtype.equalsIgnoreCase("plain")) {
                    var charset = parameters.get("charset");
                    if (charset != null) {
                        charset = charset.trim().toUpperCase();
                        if (Charset.isSupported(charset)) {
                            codecType = CodecType.getOrRegister(Charset.forName(charset));
                            break codec;
                        }
                    }
                }
                ...
                codecType = CodecType.getOrCreate(suffix);
            }

            return new MediaType(string, type, subtype, suffix, parameters, codecType);
        }
        if (error == null) {
