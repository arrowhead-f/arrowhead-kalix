 ...
 * <p>
 * A plugin attaches to one system in order to react to certain life-cycle
 * events. A plugin could, for example, be designed to react to services being
 * added to its by adding authentication mechanisms, or to bring up services
 * in response to file system events, among many other possible examples. More
 * than anything else, they provide a convenient means of packaging and reusing
 * complex behaviors that are frequently useful together.
 * <p>
 * While most plugins are expected to be designed to only be useful together
 * with a single system, they can be designed to be reusable. It should,
 * however, never be assumed unless explicitly documented.
 */
...
public interface Plugin {
     ...
     *
     * @param plug    Plug, representing the plugin's connection to its system.
     * @param plugins An unmodifiable list of plugins that are being attached
     *                to the same system as this plugin, in the order they are
     *                being attached.
     */
    default void onAttach(final Plug plug, final Set<Plugin> plugins) throws Exception {
     ...
     *
     * @param plug Plug, representing this plugin's connection to its system.
     */
     ...
     * dismissed by its {@link ArSystem}.
     * <p>
     * If this method throws an exception the plugin is detached and
     * {@link #onDetach(Plug, Throwable)} is invoked with the exception.
     *
