...
public interface HttpClientResponse {
    /**
     * Requests that the body of the HTTP response be collected into an instance
     ...
     *
     * @param class_ Class to decode response body into.
     * @param <R>    Type of {@code class_}.
     * @return Future completed when the response body has been fully received
    ...
    /**
     * Requests that the body of the HTTP response be collected into a regular
     * Java byte array ({@code byte[]}).
     *
     * @return Future completed when the response body has been fully collected
     ...
     * response body has been received in full. However, in comparison to
     * {@link #bodyAsByteArray()}, this method does not necessitate copying the
     * response body into a contiguous byte array, which in some cases can lead
     ...
     * as possible. If expecting to receive a very large response body, consider
     * using the {@link #bodyTo(Path, boolean)} method, which writes the body
     * directly to a file, as it arrives.
     *
     * @return Future completed when the response body has been fully
     ...
     * {@code "content-type"} of the response, that will be used when decoding
     * the body into a string. In any other case, UTF-8 will be assumed to be
     * adequate.
     *
     * @return Future completed when the response body becomes has been fully
    ...
    /**
     * Requests that the body of the HTTP response be written to the file at the
     * specified file system path, overwriting it if it already exists.
     *
     * @param path Path to file to contain response body.
     * @return Future completed successfully with given path only if the
     * response body is fully received and written to the file at that path.
     ...
     *               not be overwritten, but have the body of the HTTP response
     *               appended to it.
     * @return Future completed successfully with given path only if the
     * response body is fully received and written to the file at that path.
     */
